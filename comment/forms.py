...
class CommentForm(forms.Form):
    ...
    def clean(self):
        ...
        try:
            model_obj = ContentType.objects.get(model=content_type).model_class()
            con_obj = model_obj.objects.get(pk=object_id)
            self.cleaned_data['content_object'] = con_obj
            return self.cleaned_data
        except ObjectDoesNotExist:
            raise forms.ValidationError('你评论的对象不存在')
    def clean_reply_comment_id(self):
        reply_comment_id = self.cleaned_data.get('reply_comment_id')
        print('id=',reply_comment_id)
